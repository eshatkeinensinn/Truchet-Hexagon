...
class grid:
    ...
    def connected_segments(self):
        # Create a line detection object
        line_detection_obj = line_detection.detection_connected(self.grid)

        

        line_detection_obj.merge_groups()  # Detect connected segments

        colour_group = line_detection_obj.groups_collouring()

        # 10 Coulour Groups for colouring red,green,blue,orange,purple,cyan,black,white,yellow,brown svgwrite
        dwg = svgwrite.Drawing("hexagon_colour.svg", size=("210mm", "297mm"), viewBox=f"0 0 {self.width} {self.height}")
        group_0 = dwg.g(id="group_1", stroke="red", fill="none")
        group_1 = dwg.g(id="group_2", stroke="green", fill="none")
        group_2 = dwg.g(id="group_3", stroke="blue", fill="none")
        group_3 = dwg.g(id="group_4", stroke="orange", fill="none")
        group_4 = dwg.g(id="group_5", stroke="purple", fill="none")
        group_5 = dwg.g(id="group_6", stroke="cyan", fill="none")
        group_6 = dwg.g(id="group_7", stroke="black", fill ="none")
        group_7 = dwg.g(id="group_8", stroke="white", fill="none")
        group_8 = dwg.g(id="group_9", stroke="yellow", fill="none")
        group_9 = dwg.g(id="group_10", stroke="brown", fill="none")

        # Add the background if specified
        if self.background:
            dwg.add(dwg.rect(insert=(0, 0), size=("100%", "100%"), fill="black"))

        # Add the groups to the drawing
        for hexagon in self.grid:
            for segment in hexagon.connection:
                # Check if the segment is in ond of the colour groups
                for group in colour_group:
                    for seg in group.segments:
                        if hexagon.id == seg.id and segment == seg.connection:
                            # If found add the segment to the corresponding group
                            if group.id == 0:
                                print(f"Group 0: {hexagon.id} Segment: {segment}")
                                draw_segment = hexagon.draw_curve(segment)
                                for draw_line in draw_segment:
                                    # Stelle sicher, dass `line` ein `LineString` ist
                                    line_geom = LineString(draw_line)
                                    ...
                                    if not intersection.is_empty:
                                        ...
                                        if intersection.geom_type == 'MultiLineString':
                                            for seg in intersection.geoms:
                                                group_0.add(svgwrite.shapes.Polyline(points=list(seg.coords), stroke='white', fill='none', stroke_width=0.5))
                                        # Falls es sich um einen LineString handelt, füge ihn direkt hinzu
                                        elif intersection.geom_type == 'LineString':
                                            group_0.add(svgwrite.shapes.Polyline(points=list(intersection.coords), stroke='white', fill='none', stroke_width=0.5))
        ...
        dwg.save()
        print("SVG gespeichert als hex_colour.svg")
